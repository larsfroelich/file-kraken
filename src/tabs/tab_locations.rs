...
impl FileKrakenApp {
    pub fn locations_tab(&mut self, ui: &mut Ui, locations_list: Option<&Vec<Location>>) {
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            TableBuilder::new(ui)
                ...
                .body(|mut body| {
                    for i in 0..4 {
                        table_row(&mut body, "/test/path/mnt/one", &FileKrakenLocationType::Preferred , i, &mut _self.selected_location);
                    }
                    table_row(&mut body, "/test/path/mnt/one", &FileKrakenLocationType::Excluded , 4, &mut _self.selected_location);
                    for i in 0..30 {
                        table_row(&mut body, "/test/path/mnt/one", &FileKrakenLocationType::Normal , 5+i, &mut _self.selected_location);
                    }
                });
        });
    ui.button("Add");
